                        ...
                        <div class="table-responsive users-table">
                            <table class="table table-striped table-sm data-table">
                                <caption id="user_count">
                                    {{ trans_choice('usersmanagement.users-table.caption', 1, ['userscount' => $horoscopes->count()]) }}
                                </caption>
                                <thead class="thead">
                                    <tr>
                                ...
                                <tbody id="users_table">
                                    @foreach($horoscopes as $horoscope)
                                        <tr>
                                            <td>{{$horoscope->id}}</td>
                                            <td class="hidden-xs">{{$horoscope->first_name}}</td>
...
{{--                                            <td class="hidden-sm hidden-xs hidden-md">{{$user->updated_at}}</td>--}}
                                            <td>
                                                {!! Form::open(array('url' => 'users/' . $horoscope->id, 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Delete')) !!}
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-success btn-block" href="{{ URL::to('users/' . $horoscope->id) }}" data-toggle="tooltip" title="Show">
                                                    {!! trans('usersmanagement.buttons.show') !!}
                                                </a>
                                            </td>
                                            <td>
                                                <a class="btn btn-sm btn-info btn-block" href="{{ URL::to('users/' . $horoscope->id . '/edit') }}" data-toggle="tooltip" title="Edit">
...
@section('footer_scripts')
    @if ((count($horoscope) > config('usersmanagement.datatablesJsStartCount')) && config('usersmanagement.enabledDatatablesJs'))
